...
(function autoSetTitle() {
    ...
    if (lang.startsWith("ru")) {
        document.querySelector(".title-ru").style.display = "inline";
    } else if (lang.startsWith("zh")) {
        document.querySelector(".title-zh").style.display = "inline";
    } else {
        document.querySelector(".title-en").style.display = "inline";
    }
})();
...
const translations = {
    ...
    "4" : {
        zh: `【Ca清】编辑此页面且保持更新
    如有内容投稿，内容纠错以及其他问题，请经过下面的联系方式提交它们！
    【T_gas@outlook.com】 | 【Discord：t_gas】 | 【百度贴吧：Ca清（id：T_gas）】`,
    ...
    CaQing: Спасибо большое! | Теперь понятно, почему Добрая Мита смогла сохранить записку~`,
    ...
    CaQing: Thank you so much! | No wonder Kind Mita could still keep the note~
    ...
    "12.23": {
        zh: `筆叢: 为什么当善良米塔上楼之后，她身上的衣服和玩家之前设定的不一样？这是 bug 吗？
    youmэrai: 死亡会‘恢复’默认皮肤。`,
        ru: `筆叢: Почему после того, как добрый Мита подошел наверх, одежда тела отличается от одежды, которую установил игрок? Это баг?
    youmэrai: Смерть 'возвращает' обычный скин`,
        en: `筆叢: Why does Kind Mita have a different outfit when she goes upstairs than what the player set earlier? Is that a bug?
    ...
    "12.59": {
        zh: `Sutherex: 关于疯狂米塔的能力——她能生成假人吗？还是只有核心能做到？  
    rawumeerai: 她不能生成。`,
        ru: `Sutherex: может ли безумная мита спавнить манекены или это может делать только ядро?  
    rawumeerai: Не может спавнить`,
        en: `Sutherex: Does Crazy Mita have the ability to spawn mannequins? Or can only the game/core do that?  
    rawumeerai: She can’t spawn them.`
    },
    ...
    "12.61": {
        zh: `Jinsine: @umeerai，是谁设计了版本穿梭戒指？戒指上的 F 形缺口有什么意义吗？  
    umeerai: 我随便画的，本来打算改，但来不及了。`,
        ru: `Jinsine: кто рисовал кольцо для перемещений по версиям? Зачем там вырез в форме F?  
    umeerai: я просто нафристайлил, планировали менять, но не успели`,
        en: `Jinsine: Who designed the version-traveling ring? What’s the meaning behind the F-shaped notch?  
    umeerai: I just freestyled it. We planned to change it, but didn’t make it in time.`
    }
...
function setLanguage(lang) {
    for (const [key, value] of Object.entries(translations)) {
        ...
        if (el) {
            const ru = (value.ru || "")
                .split("\n")
                .map(line => `<span class="lang ru" lang="ru">${line}</span>`)
                .join("<br>");
            const other = value[lang] || "";
            el.innerHTML = `${ru}<br>${other}`;

            el.classList.remove('lang', 'zh', 'en');
            el.classList.add('lang', lang);
        }
    }

    // ⬇️ 解决语言切换后高度不更新的问题
    const expandedBlocks = document.querySelectorAll(".collapsible .content");
    expandedBlocks.forEach(block => {
        if (block.style.maxHeight && block.scrollHeight) {
